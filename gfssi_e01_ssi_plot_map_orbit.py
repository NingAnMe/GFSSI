...
def plot_image(data, out_file='test.jpg', figsize=(2.748, 2.748)):
    ditu = plt.imread('cfg/ditu.jpg')
    fig = plt.figure(figsize=figsize, dpi=1000)
    fig.figimage(ditu)
    fig.figimage(data, vmin=0, vmax=1400)
    plt.savefig(out_file)
    print('>>> :{}'.format(out_file))


def plot_map_orbit(in_file):
    ...
    for dataname in datas_.keys():
        ...
        if data is not None:
            ...
            plot_image(data, out_file=out_file)
