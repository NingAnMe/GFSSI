...
class DBUtil:
    ...
    def connector_DB_localhost(self):
        ...
        建立和数据库系统的连接
        '''
        db = MySQLdb.connect(host='localhost', user='root', passwd='mike', db='gsics', use_unicode=True, charset="utf8")
        return db
...
#         db = MySQLdb.connect(host='10.24.175.112', user='root', passwd='kingtansin', db='site_tan_db', use_unicode=True, charset="utf8")
        db = MySQLdb.connect(host='192.168.2.89', user='root', passwd='abc800', db='site_tan_db', use_unicode=True, charset="utf8")
        return db
