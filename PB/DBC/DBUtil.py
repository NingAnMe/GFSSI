...
class DBUtil:
    ...
    def __init__(self, host, user, passwd, dbname, port):
        ...
        while(1):
            try :
                self.db = MySQLdb.connect(host=host, user=user, passwd=passwd, db=dbname, port=port, use_unicode=True, charset="utf8")
                print 'CONNECT OK'
                break
            except Exception , e:
    ...
    def disconnect(self):
        ...
        try:
            ...
            print ('Sql:%s <%s> ,Close SQL error %s' % (self.HOST, self.DBNAME, e))
            return 'ERROE'
        finally:
            pass
    ...
    def droptable(self, tablename): 
        ...
        try :
            ...
            print ('Sql:Sql:%s <%s> ,Drop table %s error %s' % (self.HOST, self.DBNAME, tablename, e))
            cursor.close()
            return 'ERROR'
    ...
    def executeInsert(self, sql):
        ...
        try:
            ...
            print ('Sql:Sql:%s <%s> ,Insert data error %s' % (self.HOST, self.DBNAME, e))
            self.db.rollback()
            cursor.close()
            return 'ERROR'
        finally:
    ...
    def  executeInsertMany(self, sql, params=[]):
        ...
        if len(params) == 0:
            ...
            try:
                ...
                print ('Sql:Sql:%s <%s> ,Insert many data error %s' % (self.HOST, self.DBNAME, e))
                self.db.rollback()
                cursor.close()
                return 'ERROR'
            finally:
